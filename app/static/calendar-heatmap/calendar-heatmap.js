...
function calendarHeatmap() {
  ...
  var data = [];
  var max = null;
  var colorRange = ['#d9e7d8', '#208224'];
  var tooltipEnabled = true;
  var tooltipUnit = 'flight';
  ...
  function chart() {
    ...
    if (chart.data().length == 0) {
      ...
      max = d3.max(chart.data(), function (d) { return d.value; }); // max data value
    }
      ...
      if (chart.tooltipEnabled()) {
        (v === 3 ? enterSelection : enterSelection.merge(dayRects)).on('mouseover', function(d, i) {
          tooltip = d3.select(chart.selector())
            ...
            .style('top', function () {
              return formatWeekday(d.getDay()) * (SQUARE_LENGTH + SQUARE_PADDING) + MONTH_LABEL_PADDING * 2 +90 + 'px';
            });
        })
    ...
    function countForDate(d) {
      ...
      var match = chart.data().find(function (element, index) {
        return moment(element.key).isSame(d, 'day');
      });
      if (match) {
        count = match.value;
      }
